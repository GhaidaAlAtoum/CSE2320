...
// Allocates New Node , N a subtree size ( usaually 0 )
...
// Interface not recursive but calls the recursive 
Item STsearch(Key v) 
{
...
// find the key that has that particular rank 
Item STselect(int k)
{
	// If the tree is not empty and all keys are even the range of K starts from 0 and ends at 0 meaning there is a possiblility of a zero rank
/*if (k<0	 || k>head->N)
{
...
// Setting up the suptree of a node 
void fixN(link h)
// Fixes subtree size of h, assuming that subtrees have correct sizes
{
	int Ntemp=0;
	if( h->item %2 !=0 ) Ntemp = 1;
h->N=h->l->N + h->r->N + Ntemp;
}
...
tracePrint("Down",h);
if (h == z)
  return NEW(item, z, z, Ntemp);  // Attach red leaf
...
if (eq(v, h->item))
  ...
  if (h->l->red) {
    if (h->red)
      if (sw)
        if (siblingRed) {
          tracePrint("Case ~1l",hParent); printf(" \t\t Case 1  LEFT \n");
          hParent->red = 1;
          hParent->l->red = 0;
          hParent->r->red = 0;
          if (trace==2)
          ...
          tracePrint("Case ~2",h); printf(" \t\t Case 2 LEFT \n");
          h = rotR(h);  // Set up case ~3 after return
          }
      else if (siblingRed) {
        tracePrint("Case 1l",hParent); printf(" \t\t Case 1  1 LEFT \n");
        hParent->red = 1;
        hParent->l->red = 0;
        hParent->r->red = 0;
        if (trace==2)
      ...
      tracePrint("Case 3",h); printf(" \t\t Case 3 LEFT \n");
      h = rotR(h);
      h->red = 0;
      h->r->red = 1;
      }
  ...
  if (h->r->red) {
    if (h->red)
      if (!sw)
        if (siblingRed) {
          tracePrint("Case 1r",hParent); printf(" \t\t Case 1 RIGHT \n");
          hParent->red = 1;
          hParent->l->red = 0;
          hParent->r->red = 0;
          if (trace==2)
          ...
          tracePrint("Case 2",h); printf(" \t\t Case 2 RIGHT \n");
          h = rotL(h);  // Set up case 3 after return
          }
      else if (siblingRed) {
        tracePrint("Case ~1r",hParent); printf(" \t\t Case 1 1 RIGHT \n");
        hParent->red = 1;
        hParent->l->red = 0;
        hParent->r->red = 0;
        if (trace==2)
      ...
      tracePrint("Case ~3",h); printf(" \t\t Case 3 RIGHT \n");
      h = rotL(h);
      h->red = 0;
      h->l->red = 1;
      }
...
int SToddRankOfMax(){
	// Binary Search For the maximum value that will have the maximum rank 
	// it will take O(lgn) even in worst case since the tree is always balanced 
    if( head == z) return(STinvSelect(head->item));
	link temp = head;
	while ( temp ->r != z){
		temp = temp -> r;
	}
	return( STinvSelect(temp->item));
}
